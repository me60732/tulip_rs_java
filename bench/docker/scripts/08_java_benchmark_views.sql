-- =============================================================================
-- 08_java_benchmark_views.sql
-- Adds Java-comparison views to the existing indicator_benchmark database.
-- Does NOT recreate the database or touch existing tables/views.
-- Mirrors 07_go_benchmark_views.sql, filtered by the Java implementation
-- types.
--
-- Apply to a running database:
--   psql -U postgres -h 192.168.50.10 -p 5433 -d indicator_benchmark \
--        -f scripts/08_java_benchmark_views.sql
--
-- Implementation types written by the Java benchmarks:
--   'tulip_rs_java'  — tulip-rs called via the Panama FFM Java binding
--   'ta4j'           — github.com/ta4j/ta4j (pure Java reference)
-- =============================================================================

-- Drop in reverse-dependency order so re-running is safe
DROP VIEW IF EXISTS java_simd_asset_avg_comparison;
DROP VIEW IF EXISTS java_simd_asset_simplified_comparison;
DROP VIEW IF EXISTS java_simd_asset_performance_comparison;
DROP VIEW IF EXISTS java_simd_avg_comparison;
DROP VIEW IF EXISTS java_simd_simplified_comparison;
DROP VIEW IF EXISTS java_simd_performance_comparison;
DROP VIEW IF EXISTS java_avg_options_comparison;
DROP VIEW IF EXISTS java_performance_comparison;

-- ---------------------------------------------------------------------------
-- java_performance_comparison
-- One row per (run, indicator, stock, option-set).
-- Pivots tulip_rs_java and ta4j side by side and computes the ratio.
-- ---------------------------------------------------------------------------
CREATE VIEW java_performance_comparison AS
SELECT
    runs.id                            AS run_id,
    ...
    max(CASE WHEN res.implementation_type = 'tulip_rs_java'
             THEN res.mean_time_ns END)                          AS tulip_rs_java_mean_ns,
    max(CASE WHEN res.implementation_type = 'tulip_rs_java'
             THEN res.std_dev_ns END)                            AS tulip_rs_java_stddev_ns,
    max(CASE WHEN res.implementation_type = 'ta4j'
             THEN res.mean_time_ns END)                          AS ta4j_mean_ns,
    max(CASE WHEN res.implementation_type = 'ta4j'
             THEN res.std_dev_ns END)                            AS ta4j_stddev_ns,

    -- How many times slower is ta4j vs tulip_rs_java?
    round(
        (max(CASE WHEN res.implementation_type = 'ta4j'
        ...
        / NULLIF(
            (max(CASE WHEN res.implementation_type = 'tulip_rs_java'
                      THEN res.mean_time_ns END))::numeric,
          0),
    2)                                                           AS ta4j_to_tulip_ratio,

    -- Percentage of time saved by using tulip_rs_java instead of ta4j
    round(
        (
          (max(CASE WHEN res.implementation_type = 'ta4j'
...
-- Compare within a run, never across runs (JVM/hardware conditions differ).
-- ---------------------------------------------------------------------------
CREATE VIEW java_avg_options_comparison AS
SELECT
    runs.id                            AS run_id,
    runs.run_timestamp                 AS benchmark_date,
    (runs.system_info ->> 'hostname')  AS hostname,
    ...
    round(avg(CASE WHEN res.implementation_type = 'tulip_rs_java'
                   THEN res.mean_time_ns END))                   AS tulip_rs_java_avg_ns,
    round(avg(CASE WHEN res.implementation_type = 'ta4j'
                   THEN res.mean_time_ns END))                   AS ta4j_avg_ns,

    count(DISTINCT CASE WHEN res.implementation_type = 'tulip_rs_java'
                        THEN res.options END)                    AS tulip_options_count,
    count(DISTINCT CASE WHEN res.implementation_type = 'ta4j'
                        THEN res.options END)                    AS ta4j_options_count,

    round(
        avg(CASE WHEN res.implementation_type = 'ta4j'
                 THEN res.mean_time_ns END)
        / NULLIF(
            avg(CASE WHEN res.implementation_type = 'tulip_rs_java'
                     THEN res.mean_time_ns END),
          0),
    2)                                                           AS ta4j_to_tulip_ratio,
...
-- Compares the tulip_rs_java SIMD-batched code paths (written by the
-- bench runner as 'tulip_rs_java_simd_by_options' / '..._simd_by_assets')
-- against the plain 'tulip_rs_java' baseline (and, for the by-assets case,
-- also against 'ta4j'). Mirrors the Rust rust_simd_* view chain:
...
-- ---------------------------------------------------------------------------
-- java_simd_performance_comparison
...
-- tulip_rs_java once per option set for that same stock.
-- ---------------------------------------------------------------------------
CREATE VIEW java_simd_performance_comparison AS
SELECT
    runs.id                            AS run_id,
    runs.run_timestamp                 AS benchmark_date,
    (runs.system_info ->> 'hostname')  AS hostname,
    ...
    sum(CASE WHEN res.implementation_type = 'tulip_rs_java'
             THEN res.mean_time_ns END)                          AS tulip_total_mean_time_ns,
    count(CASE WHEN res.implementation_type = 'tulip_rs_java'
               THEN 1 END)                                       AS tulip_options_count,
    max(CASE WHEN res.implementation_type = 'tulip_rs_java_simd_by_options'
...
JOIN benchmark_results res ON runs.id = res.run_id
JOIN indicators ind        ON res.indicator_id = ind.id
WHERE res.implementation_type IN ('tulip_rs_java', 'tulip_rs_java_simd_by_options')
GROUP BY
    runs.id, runs.run_timestamp, runs.system_info,
    ind.name, res.stock_symbol, res.data_source, res.input_size
HAVING
    sum(CASE WHEN res.implementation_type = 'tulip_rs_java' THEN 1 ELSE 0 END) > 0
    AND sum(CASE WHEN res.implementation_type = 'tulip_rs_java_simd_by_options' THEN 1 ELSE 0 END) > 0
ORDER BY runs.run_timestamp DESC, ind.name, res.stock_symbol;

CREATE VIEW java_simd_simplified_comparison AS
...
FROM java_simd_simplified_comparison
GROUP BY run_id, benchmark_date, hostname, indicator_name
ORDER BY benchmark_date DESC, indicator_name;

-- ---------------------------------------------------------------------------
-- java_simd_asset_performance_comparison
-- SIMD-by-assets: one option set, every loaded stock processed together in a
-- single call. Compared against the summed serial cost of running
-- tulip_rs_java / ta4j once per stock for that same option set.
-- ---------------------------------------------------------------------------
CREATE VIEW java_simd_asset_performance_comparison AS
SELECT
    runs.id                            AS run_id,
    runs.run_timestamp                 AS benchmark_date,
    (runs.system_info ->> 'hostname')  AS hostname,
    ...
    sum(CASE WHEN res.implementation_type = 'ta4j'
             THEN res.mean_time_ns ELSE 0 END)                  AS ta4j_total_mean_time_ns,
    avg(CASE WHEN res.implementation_type = 'tulip_rs_java_simd_by_assets'
    ...
    round(
        ...
        / avg(CASE WHEN res.implementation_type = 'tulip_rs_java_simd_by_assets'
                   THEN res.mean_time_ns END)) * 100,
    2)                                                           AS simd_asset_vs_ta4j_improvement_pct

FROM benchmark_runs runs
JOIN benchmark_results res ON runs.id = res.run_id
JOIN indicators ind        ON res.indicator_id = ind.id
WHERE res.implementation_type IN ('tulip_rs_java', 'ta4j', 'tulip_rs_java_simd_by_assets')
GROUP BY
    runs.id, runs.run_timestamp, runs.system_info,
...
HAVING
    ...
    AND (
        sum(CASE WHEN res.implementation_type = 'tulip_rs_java' THEN 1 ELSE 0 END) > 0
        OR sum(CASE WHEN res.implementation_type = 'ta4j' THEN 1 ELSE 0 END) > 0
    )
ORDER BY runs.run_timestamp DESC, ind.name, res.options;

CREATE VIEW java_simd_asset_simplified_comparison AS
SELECT run_id, benchmark_date, hostname, indicator_name, data_source,
    round(avg(tulip_total_mean_time_ns))                        AS tulip_avg_total_time_ns,
    round(avg(ta4j_total_mean_time_ns))                         AS ta4j_avg_total_time_ns,
    ...
    round(avg(simd_asset_vs_ta4j_improvement_pct), 2)           AS avg_simd_asset_vs_ta4j_improvement_pct
FROM java_simd_asset_performance_comparison
GROUP BY run_id, benchmark_date, hostname, indicator_name, data_source
ORDER BY benchmark_date DESC, indicator_name;

CREATE VIEW java_simd_asset_avg_comparison AS
SELECT run_id, benchmark_date, hostname, indicator_name,
    round(avg(tulip_avg_total_time_ns))                         AS tulip_overall_avg_time_ns,
    round(avg(ta4j_avg_total_time_ns))                          AS ta4j_overall_avg_time_ns,
    ...
    round(avg(avg_simd_asset_vs_ta4j_improvement_pct), 2)       AS overall_simd_asset_vs_ta4j_improvement_pct
FROM java_simd_asset_simplified_comparison
GROUP BY run_id, benchmark_date, hostname, indicator_name
ORDER BY benchmark_date DESC, indicator_name;
