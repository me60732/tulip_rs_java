...
public final class BopExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Bop.minData(options) + 100;
        ...
        Outcome oc = Bop.indicator(series, options, null);
        double[] fullBop;
        try (Result res = oc.result()) {
            fullBop = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Bop.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continued = br.toDoubleArray(0);
                double[] tail = Arrays.copyOfRange(fullBop, fullBop.length - continued.length,
                        fullBop.length);
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Bop.ID);
                State rs = Bop.deserializeState(blob);
        ...
        double[][] scaled = slices(new double[][] {
                Demo.scale(open, 1.2), Demo.scale(high, 1.2), Demo.scale(low, 1.2), Demo.scale(close, 1.2)}, 0, n);
        double[][][] assets = {series, scaled};
        try (SimdResult sim = Bop.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Bop.indicator(assets[i], options, null);
