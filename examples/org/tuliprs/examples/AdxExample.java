...
public final class AdxExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Adx.minData(options) + 100;
        ...
        Outcome oc = Adx.indicator(series, options, new boolean[] {true, true, true});
        double[] fullAdx;
        try (Result res = oc.result()) {
            fullAdx = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Adx.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continued = br.toDoubleArray(0);
                double[] tail = Arrays.copyOfRange(fullAdx, fullAdx.length - continued.length,
                        fullAdx.length);
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Adx.ID);
                State rs = Adx.deserializeState(blob);
        ...
        try (SimdResult sim = Adx.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Adx.indicator(assets[i], options, null);
        ...
        try (SimdResult sim = Adx.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Adx.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
