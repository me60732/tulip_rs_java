...
public final class VhfExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Vhf.minData(options) + 100;
        ...
        Outcome oc = Vhf.indicator(series, options, new boolean[] {true, true, true, true});
        double[] fullVhf;
        try (Result res = oc.result()) {
            fullVhf = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Vhf.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continuedVhf = br.toDoubleArray(0);
                double[] tailVhf = Arrays.copyOfRange(fullVhf, fullVhf.length - continuedVhf.length,
                        fullVhf.length);
                c.match("partial+continued vhf equals full recompute", Demo.same(tailVhf, continuedVhf));
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Vhf.ID);
                State rs = Vhf.deserializeState(blob);
                ...
                try (rs; cl) {
                    ...
                    try (b1; b2; b3) {
                        c.match("deserialized state continues identically vhf",
                                Demo.same(b1.toDoubleArray(0), b2.toDoubleArray(0)));
                        c.match("cloned state continues identically vhf",
        ...
        try (SimdResult sim = Vhf.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Vhf.indicator(assets[i], options, null);
        ...
        try (SimdResult sim = Vhf.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Vhf.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
