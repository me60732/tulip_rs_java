...
public final class UltoscExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Ultosc.minData(options) + 100;
        ...
        // ---- full compute --------------------------------------------------
        System.out.println("\n=== full calculation ===");
        Outcome oc = Ultosc.indicator(series, options);
        double[] fullUltosc;
        try (Result res = oc.result()) {
            fullUltosc = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Ultosc.indicator(slices(series, 0, partial), options);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continued = br.toDoubleArray(0);
                double[] tail = Arrays.copyOfRange(fullUltosc, fullUltosc.length - continued.length,
                        fullUltosc.length);
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Ultosc.ID);
                State rs = Ultosc.deserializeState(blob);
        ...
        try (SimdResult sim = Ultosc.simdByAssets(assets, options)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Ultosc.indicator(assets[i], options);
        ...
        // ---- SIMD by options -------------------------------------------------
        System.out.println("\n=== SIMD by options (N=4) ===");
        double[][] optSets = {{5, 10, 20}, {7, 14, 28}, {10, 20, 40}, {14, 28, 56}};
        try (SimdResult sim = Ultosc.simdByOptions(series, optSets)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Ultosc.indicator(series, optSets[i]);
                try (Result ind = r.result(); State st2 = r.state()) {
