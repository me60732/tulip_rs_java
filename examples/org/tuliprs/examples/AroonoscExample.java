...
public final class AroonoscExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Aroonosc.minData(options) + 100;
        ...
        Outcome oc = Aroonosc.indicator(series, options, null);
        double[] fullAroonosc;
        try (Result res = oc.result()) {
            fullAroonosc = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Aroonosc.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continued = br.toDoubleArray(0);
                double[] tail = Arrays.copyOfRange(fullAroonosc, fullAroonosc.length - continued.length,
                        fullAroonosc.length);
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Aroonosc.ID);
                State rs = Aroonosc.deserializeState(blob);
        ...
        try (SimdResult sim = Aroonosc.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Aroonosc.indicator(assets[i], options, null);
        ...
        try (SimdResult sim = Aroonosc.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Aroonosc.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
