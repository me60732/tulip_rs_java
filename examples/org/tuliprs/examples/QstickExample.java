...
public final class QstickExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Qstick.minData(options) + 100;
        String[] names = info.inputs().toArray(new String[0]);
        double[][] series = Demo.seriesFor(names, n);
        double[] open = series[0], close = series[1];

        // ---- full compute --------------------------------------------------
        System.out.println("\n=== full calculation (all optional outputs) ===");
        Outcome oc = Qstick.indicator(series, options, null);
        double[] fullQstick;
        try (Result res = oc.result()) {
            fullQstick = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Qstick.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continuedQstick = br.toDoubleArray(0);
                double[] tailQstick = Arrays.copyOfRange(fullQstick, fullQstick.length - continuedQstick.length,
                        fullQstick.length);
                c.match("partial+continued equals full recompute", Demo.same(tailQstick, continuedQstick));
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Qstick.ID);
                State rs = Qstick.deserializeState(blob);
        ...
        double[][] scaled = slices(new double[][] {
                Demo.scale(open, 1.2), Demo.scale(close, 1.2)}, 0, n);
        double[][][] assets = {series, scaled};
        try (SimdResult sim = Qstick.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Qstick.indicator(assets[i], options, null);
        ...
        try (SimdResult sim = Qstick.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Qstick.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
