 ...
 */
public final class NatrExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Natr.minData(options) + 100;
        ...
        Outcome oc = Natr.indicator(series, options, new boolean[] {true, true});
        double[] fullNatr;
        double[] fullAtr;
        double[] fullTr;
        try (Result res = oc.result()) {
            fullNatr = res.toDoubleArray(0);
            fullAtr = res.toDoubleArray(1);
            fullTr = res.toDoubleArray(2);
        ...
        int partial = n - 50;
        Outcome p = Natr.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n), new boolean[] {true, true});
            try (br) {
                double[] continuedNatr = br.toDoubleArray(0);
                double[] continuedAtr = br.toDoubleArray(1);
                double[] continuedTr = br.toDoubleArray(2);
                double[] tailNatr = Arrays.copyOfRange(fullNatr, fullNatr.length - continuedNatr.length,
                        fullNatr.length);
                double[] tailAtr = Arrays.copyOfRange(fullAtr, fullAtr.length - continuedAtr.length,
                        fullAtr.length);
                double[] tailTr = Arrays.copyOfRange(fullTr, fullTr.length - continuedTr.length,
                        fullTr.length);
                c.match("partial+continued natr equals full recompute", Demo.same(tailNatr, continuedNatr));
                c.match("partial+continued atr equals full recompute", Demo.same(tailAtr, continuedAtr));
                c.match("partial+continued tr equals full recompute", Demo.same(tailTr, continuedTr));
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Natr.ID);
                State rs = Natr.deserializeState(blob);
                ...
                try (rs; cl) {
                    ...
                    try (b1; b2; b3) {
                        c.match("deserialized state continues identically natr",
                                Demo.same(b1.toDoubleArray(0), b2.toDoubleArray(0)));
                        c.match("deserialized state continues identically atr",
                                Demo.same(b1.toDoubleArray(1), b2.toDoubleArray(1)));
                        c.match("deserialized state continues identically tr",
                                Demo.same(b1.toDoubleArray(2), b2.toDoubleArray(2)));
                        c.match("cloned state continues identically natr",
                                Demo.same(b1.toDoubleArray(0), b3.toDoubleArray(0)));
                        c.match("cloned state continues identically atr",
                                Demo.same(b1.toDoubleArray(1), b3.toDoubleArray(1)));
                        c.match("cloned state continues identically tr",
        ...
        try (SimdResult sim = Natr.simdByAssets(assets, options, new boolean[] {true, true})) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Natr.indicator(assets[i], options, new boolean[] {true, true});
                try (Result ind = r.result(); State st2 = r.state()) {
                    c.match("SIMD asset " + (i + 1) + " natr equals individual", Demo.sameTol(
                            sim.toDoubleArray(i, 0), ind.toDoubleArray(0), 1e-6, 1e-9));
                    c.match("SIMD asset " + (i + 1) + " atr equals individual", Demo.sameTol(
                            sim.toDoubleArray(i, 1), ind.toDoubleArray(1), 1e-6, 1e-9));
                    c.match("SIMD asset " + (i + 1) + " tr equals individual", Demo.sameTol(
        ...
        try (SimdResult sim = Natr.simdByOptions(series, optSets, new boolean[] {true, true})) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Natr.indicator(series, optSets[i], new boolean[] {true, true});
                try (Result ind = r.result(); State st2 = r.state()) {
                    c.match("SIMD option set " + (i + 1) + " natr equals individual", Demo.sameTol(
                            sim.toDoubleArray(i, 0), ind.toDoubleArray(0), 1e-6, 1e-9));
                    c.match("SIMD option set " + (i + 1) + " atr equals individual", Demo.sameTol(
                            sim.toDoubleArray(i, 1), ind.toDoubleArray(1), 1e-6, 1e-9));
                    c.match("SIMD option set " + (i + 1) + " tr equals individual", Demo.sameTol(
                            sim.toDoubleArray(i, 2), ind.toDoubleArray(2), 1e-6, 1e-9));
                }
            }
        }
