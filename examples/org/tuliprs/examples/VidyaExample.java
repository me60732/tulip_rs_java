 ...
 */
public final class VidyaExample {

    public static void main(String[] args) {
        Demo.Check c = new Demo.Check();
        double[] options = {14.0, 30.0, 0.2}; // short_period, long_period, alpha

        Info info = Vidya.info();
        ...
        int n = 2 * (int) Vidya.minData(options) + 100;
        ...
        Outcome oc = Vidya.indicator(series, options, new boolean[] {true, true, true});
        double[] fullVidya;
        try (Result res = oc.result()) {
            fullVidya = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Vidya.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continuedVidya = br.toDoubleArray(0);
                double[] tailVidya = Arrays.copyOfRange(fullVidya, fullVidya.length - continuedVidya.length,
                        fullVidya.length);
                c.match("partial+continued vidya equals full recompute", Demo.same(tailVidya, continuedVidya));
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Vidya.ID);
                State rs = Vidya.deserializeState(blob);
                ...
                try (rs; cl) {
                    ...
                    try (b1; b2; b3) {
                        c.match("deserialized state continues identically vidya",
                                Demo.same(b1.toDoubleArray(0), b2.toDoubleArray(0)));
                        c.match("cloned state continues identically vidya",
        ...
        try (SimdResult sim = Vidya.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Vidya.indicator(assets[i], options, null);
        ...
        // ---- SIMD by options -------------------------------------------------
        System.out.println("\n=== SIMD by options (N=4) ===");
        double[][] optSets = {{10, 20, 0.2}, {14, 30, 0.2}, {20, 40, 0.2}, {25, 50, 0.2}};
        try (SimdResult sim = Vidya.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Vidya.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
