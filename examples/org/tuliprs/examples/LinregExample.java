...
public final class LinregExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Linreg.minData(options) + 100;
        ...
        Outcome oc = Linreg.indicator(series, options, new boolean[] {true, true});
        double[] fullLinreg;
        try (Result res = oc.result()) {
            fullLinreg = res.toDoubleArray(0);
        ...
        try (Result pr = p.result(); State pst = p.state()) {
            ...
            try (br) {
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Linreg.ID);
                State rs = Linreg.deserializeState(blob);
        ...
        try (SimdResult sim = Linreg.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Linreg.indicator(assets[i], options, null);
        ...
        try (SimdResult sim = Linreg.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Linreg.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
