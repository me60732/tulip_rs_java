...
public final class NviExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Nvi.minData(options) + 100;
        ...
        // ---- full compute --------------------------------------------------
        System.out.println("\n=== full calculation ===");
        Outcome oc = Nvi.indicator(series, options);
        double[] fullNvi;
        try (Result res = oc.result()) {
            fullNvi = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Nvi.indicator(slices(series, 0, partial), options);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continuedNvi = br.toDoubleArray(0);
                double[] tailNvi = Arrays.copyOfRange(fullNvi, fullNvi.length - continuedNvi.length,
                        fullNvi.length);
                c.match("partial+continued equals full recompute", Demo.same(tailNvi, continuedNvi));
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Nvi.ID);
                State rs = Nvi.deserializeState(blob);
        ...
        try (SimdResult sim = Nvi.simdByAssets(assets, options)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Nvi.indicator(assets[i], options);
        ...
        // ---- SIMD by options -------------------------------------------------
        System.out.println("\n=== SIMD by options (N/A) ===");
        System.out.println("  NVI has zero options, so SimdByOptions is not available.");
