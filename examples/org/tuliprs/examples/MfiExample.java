...
public final class MfiExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Mfi.minData(options) + 100;
        ...
        Outcome oc = Mfi.indicator(series, options, new boolean[] {true});
        double[] fullMfi;
        try (Result res = oc.result()) {
            fullMfi = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Mfi.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continued = br.toDoubleArray(0);
                double[] tail = Arrays.copyOfRange(fullMfi, fullMfi.length - continued.length,
                        fullMfi.length);
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Mfi.ID);
                State rs = Mfi.deserializeState(blob);
        ...
        try (SimdResult sim = Mfi.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Mfi.indicator(assets[i], options, null);
        ...
        try (SimdResult sim = Mfi.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Mfi.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
