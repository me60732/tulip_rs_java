...
public final class MdExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Md.minData(options) + 100;
        ...
        Outcome oc = Md.indicator(series, options, new boolean[] {true});
        double[] fullMd;
        try (Result res = oc.result()) {
            fullMd = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Md.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continued = br.toDoubleArray(0);
                double[] tail = Arrays.copyOfRange(fullMd, fullMd.length - continued.length,
                        fullMd.length);
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Md.ID);
                State rs = Md.deserializeState(blob);
        ...
        try (SimdResult sim = Md.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Md.indicator(assets[i], options, null);
        ...
        try (SimdResult sim = Md.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Md.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
