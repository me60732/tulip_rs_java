 ...
 */
public final class VolatilityExample {

    public static void main(String[] args) {
        Demo.Check c = new Demo.Check();
        double[] options = {14.0}; // period

        Info info = Volatility.info();
        ...
        int n = 2 * (int) Volatility.minData(options) + 100;
        ...
        Outcome oc = Volatility.indicator(series, options, new boolean[] {true, true});
        double[] fullVolatility;
        try (Result res = oc.result()) {
            fullVolatility = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Volatility.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continuedVolatility = br.toDoubleArray(0);
                double[] tailVolatility = Arrays.copyOfRange(fullVolatility, fullVolatility.length - continuedVolatility.length,
                        fullVolatility.length);
                c.match("partial+continued volatility equals full recompute", Demo.same(tailVolatility, continuedVolatility));
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Volatility.ID);
                State rs = Volatility.deserializeState(blob);
                ...
                try (rs; cl) {
                    ...
                    try (b1; b2; b3) {
                        c.match("deserialized state continues identically volatility",
                                Demo.same(b1.toDoubleArray(0), b2.toDoubleArray(0)));
                        c.match("cloned state continues identically volatility",
        ...
        try (SimdResult sim = Volatility.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Volatility.indicator(assets[i], options, null);
        ...
        try (SimdResult sim = Volatility.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Volatility.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
