...
public final class MedpriceExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Medprice.minData(options) + 100;
        ...
        // ---- full compute --------------------------------------------------
        System.out.println("\n=== full calculation ===");
        Outcome oc = Medprice.indicator(series, options);
        double[] fullMedprice;
        try (Result res = oc.result()) {
            fullMedprice = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Medprice.indicator(slices(series, 0, partial), options);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continued = br.toDoubleArray(0);
                double[] tail = Arrays.copyOfRange(fullMedprice, fullMedprice.length - continued.length,
                        fullMedprice.length);
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Medprice.ID);
                State rs = Medprice.deserializeState(blob);
        ...
        try (SimdResult sim = Medprice.simdByAssets(assets, options)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Medprice.indicator(assets[i], options);
