 ...
 */
public final class TrendmodeExample {

    public static void main(String[] args) {
        Demo.Check c = new Demo.Check();
        double[] options = {0.33}; // alpha

        Info info = Trendmode.info();
        ...
        int n = 2 * (int) Trendmode.minData(options) + 100;
        ...
        Outcome oc = Trendmode.indicator(series, options, new boolean[] {true, true});
        double[] fullTrendmode;
        try (Result res = oc.result()) {
            fullTrendmode = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Trendmode.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continued = br.toDoubleArray(0);
                double[] tail = Arrays.copyOfRange(fullTrendmode, fullTrendmode.length - continued.length,
                        fullTrendmode.length);
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Trendmode.ID);
                State rs = Trendmode.deserializeState(blob);
        ...
        try (SimdResult sim = Trendmode.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Trendmode.indicator(assets[i], options, null);
        ...
        // ---- SIMD by options -------------------------------------------------
        System.out.println("\n=== SIMD by options (N=4) ===");
        double[][] optSets = {{0.25}, {0.33}, {0.5}, {0.67}};
        try (SimdResult sim = Trendmode.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Trendmode.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
