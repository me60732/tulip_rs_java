...
public final class TsfExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Tsf.minData(options) + 100;
        ...
        Outcome oc = Tsf.indicator(series, options, new boolean[] {true, true, true});
        double[] fullTsf;
        try (Result res = oc.result()) {
            fullTsf = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Tsf.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continued = br.toDoubleArray(0);
                double[] tail = Arrays.copyOfRange(fullTsf, fullTsf.length - continued.length,
                        fullTsf.length);
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Tsf.ID);
                State rs = Tsf.deserializeState(blob);
        ...
        try (SimdResult sim = Tsf.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Tsf.indicator(assets[i], options, null);
        ...
        try (SimdResult sim = Tsf.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Tsf.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
