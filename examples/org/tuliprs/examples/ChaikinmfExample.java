...
public final class ChaikinmfExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Chaikinmf.minData(options) + 100;
        String[] names = info.inputs().toArray(new String[0]);
        double[][] series = Demo.seriesFor(names, n);
        double[] high = series[0], low = series[1], close = series[2], volume = series[3];

        // ---- full compute --------------------------------------------------
        System.out.println("\n=== full calculation (all optional outputs) ===");
        Outcome oc = Chaikinmf.indicator(series, options, null);
        double[] fullChaikinmf;
        try (Result res = oc.result()) {
            fullChaikinmf = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Chaikinmf.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continued = br.toDoubleArray(0);
                double[] tail = Arrays.copyOfRange(fullChaikinmf, fullChaikinmf.length - continued.length,
                        fullChaikinmf.length);
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Chaikinmf.ID);
                State rs = Chaikinmf.deserializeState(blob);
        ...
        double[][] scaled = slices(new double[][] {
                Demo.scale(high, 1.2), Demo.scale(low, 1.2), Demo.scale(close, 1.2), Demo.scale(volume, 1.2)}, 0, n);
        double[][][] assets = {series, scaled};
        try (SimdResult sim = Chaikinmf.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Chaikinmf.indicator(assets[i], options, null);
        ...
        // ---- SIMD by options -------------------------------------------------
        System.out.println("\n=== SIMD by options (N=4) ===");
        double[][] optSets = {{10}, {20}, {30}, {50}};
        try (SimdResult sim = Chaikinmf.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Chaikinmf.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
