...
public final class KvoExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Kvo.minData(options) + 100;
        ...
        Outcome oc = Kvo.indicator(series, options, new boolean[] {true, true});
        double[] fullKvo;
        try (Result res = oc.result()) {
            fullKvo = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Kvo.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continued = br.toDoubleArray(0);
                double[] tail = Arrays.copyOfRange(fullKvo, fullKvo.length - continued.length,
                        fullKvo.length);
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Kvo.ID);
                State rs = Kvo.deserializeState(blob);
        ...
        try (SimdResult sim = Kvo.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Kvo.indicator(assets[i], options, null);
        ...
        // ---- SIMD by options -------------------------------------------------
        System.out.println("\n=== SIMD by options (N=4) ===");
        double[][] optSets = {{5, 35}, {10, 35}, {5, 50}, {10, 50}};
        try (SimdResult sim = Kvo.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Kvo.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
