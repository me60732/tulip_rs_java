...
public final class WillrExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Willr.minData(options) + 100;
        ...
        Outcome oc = Willr.indicator(series, options, new boolean[] {true, true});
        double[] fullWillr;
        double[] fullMin;
        double[] fullMax;
        try (Result res = oc.result()) {
            fullWillr = res.toDoubleArray(0);
            fullMin = res.toDoubleArray(1);
            fullMax = res.toDoubleArray(2);
        ...
        int partial = n - 50;
        Outcome p = Willr.indicator(slices(series, 0, partial), options, new boolean[] {true, true});
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n), new boolean[] {true, true});
            try (br) {
                double[] continuedWillr = br.toDoubleArray(0);
                double[] continuedMin = br.toDoubleArray(1);
                double[] continuedMax = br.toDoubleArray(2);
                double[] tailWillr = Arrays.copyOfRange(fullWillr, fullWillr.length - continuedWillr.length,
                        fullWillr.length);
                double[] tailMin = Arrays.copyOfRange(fullMin, fullMin.length - continuedMin.length,
                        fullMin.length);
                double[] tailMax = Arrays.copyOfRange(fullMax, fullMax.length - continuedMax.length,
                        fullMax.length);
                c.match("partial+continued willr equals full recompute", Demo.same(tailWillr, continuedWillr));
                c.match("partial+continued min equals full recompute", Demo.same(tailMin, continuedMin));
                c.match("partial+continued max equals full recompute", Demo.same(tailMax, continuedMax));
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Willr.ID);
                State rs = Willr.deserializeState(blob);
                ...
                try (rs; cl) {
                    ...
                    try (b1; b2; b3) {
                        c.match("deserialized state continues identically willr",
                                Demo.same(b1.toDoubleArray(0), b2.toDoubleArray(0)));
                        c.match("cloned state continues identically willr",
                                Demo.same(b1.toDoubleArray(0), b3.toDoubleArray(0)));
                        c.match("cloned state continues identically min",
                                Demo.same(b1.toDoubleArray(1), b3.toDoubleArray(1)));
                        c.match("cloned state continues identically max",
        ...
        try (SimdResult sim = Willr.simdByAssets(assets, options, new boolean[] {true, true})) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Willr.indicator(assets[i], options, new boolean[] {true, true});
                try (Result ind = r.result(); State st2 = r.state()) {
                    c.match("SIMD asset " + (i + 1) + " willr equals individual", Demo.sameTol(
                            sim.toDoubleArray(i, 0), ind.toDoubleArray(0), 1e-6, 1e-9));
                    c.match("SIMD asset " + (i + 1) + " min equals individual", Demo.sameTol(
                            sim.toDoubleArray(i, 1), ind.toDoubleArray(1), 1e-6, 1e-9));
                    c.match("SIMD asset " + (i + 1) + " max equals individual", Demo.sameTol(
        ...
        try (SimdResult sim = Willr.simdByOptions(series, optSets, new boolean[] {true, true})) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Willr.indicator(series, optSets[i], new boolean[] {true, true});
                try (Result ind = r.result(); State st2 = r.state()) {
                    c.match("SIMD option set " + (i + 1) + " willr equals individual", Demo.sameTol(
                            sim.toDoubleArray(i, 0), ind.toDoubleArray(0), 1e-6, 1e-9));
                    c.match("SIMD option set " + (i + 1) + " min equals individual", Demo.sameTol(
                            sim.toDoubleArray(i, 1), ind.toDoubleArray(1), 1e-6, 1e-9));
                    c.match("SIMD option set " + (i + 1) + " max equals individual", Demo.sameTol(
                            sim.toDoubleArray(i, 2), ind.toDoubleArray(2), 1e-6, 1e-9));
                }
            }
        }
