...
public final class TrimaExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Trima.minData(options) + 100;
        ...
        Outcome oc = Trima.indicator(series, options);
        double[] fullTrima;
        try (Result res = oc.result()) {
            fullTrima = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Trima.indicator(slices(series, 0, partial), options);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continued = br.toDoubleArray(0);
                double[] tail = Arrays.copyOfRange(fullTrima, fullTrima.length - continued.length,
                        fullTrima.length);
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Trima.ID);
                State rs = Trima.deserializeState(blob);
        ...
        try (SimdResult sim = Trima.simdByAssets(assets, options)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Trima.indicator(assets[i], options);
        ...
        try (SimdResult sim = Trima.simdByOptions(series, optSets)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Trima.indicator(series, optSets[i]);
                try (Result ind = r.result(); State st2 = r.state()) {
    ...
    /** Output names in order (mandatory outputs only for TRIMA). */
