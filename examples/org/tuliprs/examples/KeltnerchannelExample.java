 ...
 */
public final class KeltnerchannelExample {

    public static void main(String[] args) {
        Demo.Check c = new Demo.Check();
        double[] options = {20.0, 2.0}; // period, step

        Info info = Keltnerchannel.info();
        ...
        int n = 2 * (int) Keltnerchannel.minData(options) + 100;
        ...
        Outcome oc = Keltnerchannel.indicator(series, options, new boolean[] {true, true});
        double[] fullLower;
        double[] fullMiddle;
        double[] fullUpper;
        ...
        int partial = n - 50;
        Outcome p = Keltnerchannel.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            ...
            try (br) {
                ...
                double[] tailUpper = Arrays.copyOfRange(fullUpper, fullUpper.length - continuedUpper.length,
                        fullUpper.length);
                c.match("partial+continued lower equals full recompute", Demo.same(tailLower, continuedLower));
                c.match("partial+continued middle equals full recompute", Demo.same(tailMiddle, continuedMiddle));
                c.match("partial+continued upper equals full recompute", Demo.same(tailUpper, continuedUpper));
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Keltnerchannel.ID);
                State rs = Keltnerchannel.deserializeState(blob);
        ...
        try (SimdResult sim = Keltnerchannel.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Keltnerchannel.indicator(assets[i], options, null);
        ...
        // ---- SIMD by options -------------------------------------------------
        System.out.println("\n=== SIMD by options (N=4) ===");
        double[][] optSets = {{15, 2}, {20, 2}, {25, 2}, {30, 2}};
        try (SimdResult sim = Keltnerchannel.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Keltnerchannel.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
