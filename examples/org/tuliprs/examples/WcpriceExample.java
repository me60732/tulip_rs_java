...
public final class WcpriceExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Wcprice.minData(options) + 100;
        ...
        Outcome oc = Wcprice.indicator(series, options);
        double[] fullWcprice;
        try (Result res = oc.result()) {
            fullWcprice = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Wcprice.indicator(slices(series, 0, partial), options);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continued = br.toDoubleArray(0);
                double[] tail = Arrays.copyOfRange(fullWcprice, fullWcprice.length - continued.length,
                        fullWcprice.length);
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Wcprice.ID);
                State rs = Wcprice.deserializeState(blob);
        ...
        try (SimdResult sim = Wcprice.simdByAssets(assets, options)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Wcprice.indicator(assets[i], options);
