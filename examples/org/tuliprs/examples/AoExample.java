...
public final class AoExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Ao.minData(options) + 100;
        ...
        Outcome oc = Ao.indicator(series, options, null);
        double[] fullAo;
        try (Result res = oc.result()) {
            fullAo = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Ao.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continued = br.toDoubleArray(0);
                double[] tail = Arrays.copyOfRange(fullAo, fullAo.length - continued.length,
                        fullAo.length);
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Ao.ID);
                State rs = Ao.deserializeState(blob);
        ...
        try (SimdResult sim = Ao.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Ao.indicator(assets[i], options, null);
