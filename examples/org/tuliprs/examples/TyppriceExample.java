 ...
 * (serialize / deserialize / duplicate) — the Java mirror of the Go/C examples,
 * with every step verified.
 */
public final class TyppriceExample {

    public static void main(String[] args) {
        Demo.Check c = new Demo.Check();
        double[] options = new double[0];

        Info info = Typprice.info();
        ...
        int n = 2 * (int) Typprice.minData(options) + 100;
        ...
        // ---- full compute --------------------------------------------------
        System.out.println("\n=== full calculation ===");
        Outcome oc = Typprice.indicator(series, options);
        double[] fullTypprice;
        try (Result res = oc.result()) {
            fullTypprice = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Typprice.indicator(slices(series, 0, partial), options);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continued = br.toDoubleArray(0);
                double[] tail = Arrays.copyOfRange(fullTypprice, fullTypprice.length - continued.length,
                        fullTypprice.length);
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Typprice.ID);
                State rs = Typprice.deserializeState(blob);
        ...
        try (SimdResult sim = Typprice.simdByAssets(assets, options)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Typprice.indicator(assets[i], options);
