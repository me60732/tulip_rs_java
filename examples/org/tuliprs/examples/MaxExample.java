...
public final class MaxExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Max.minData(options) + 100;
        ...
        // ---- full compute --------------------------------------------------
        System.out.println("\n=== full calculation ===");
        Outcome oc = Max.indicator(series, options, null);
        double[] fullMax;
        try (Result res = oc.result()) {
            fullMax = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Max.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continuedMax = br.toDoubleArray(0);
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Max.ID);
                State rs = Max.deserializeState(blob);
                ...
                try (rs; cl) {
                    ...
                    try (b1; b2; b3) {
                        c.match("deserialized state continues identically max",
        ...
        try (SimdResult sim = Max.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Max.indicator(assets[i], options, null);
        ...
        try (SimdResult sim = Max.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Max.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
