...
public final class AroonExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Aroon.minData(options) + 100;
        ...
        Outcome oc = Aroon.indicator(series, options, null);
        ...
        int partial = n - 50;
        Outcome p = Aroon.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continuedUp = br.toDoubleArray(0);
                double[] continuedDown = br.toDoubleArray(1);

                // Full recompute to compare tails
                Outcome full = Aroon.indicator(series, options, null);
                try (Result fres = full.result()) {
                    double[] fullUp = fres.toDoubleArray(0);
                    double[] fullDown = fres.toDoubleArray(1);
                    c.match("partial+continued aroon_up equals full recompute",
                            Demo.same(Arrays.copyOfRange(fullUp, fullUp.length - continuedUp.length, fullUp.length), continuedUp));
                    c.match("partial+continued aroon_down equals full recompute",
                            Demo.same(Arrays.copyOfRange(fullDown, fullDown.length - continuedDown.length, fullDown.length), continuedDown));
                }
                full.state().close();
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Aroon.ID);
                State rs = Aroon.deserializeState(blob);
                ...
                try (rs; cl) {
                    ...
                    try (b1; b2; b3) {
                        c.match("deserialized state continues identically (up)",
                                Demo.same(b1.toDoubleArray(0), b2.toDoubleArray(0)));
                        c.match("cloned state continues identically (up)",
        ...
        try (SimdResult sim = Aroon.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Aroon.indicator(assets[i], options, null);
                try (Result ind = r.result(); State st2 = r.state()) {
                    c.match("SIMD asset " + (i + 1) + " aroon_up equals individual", Demo.sameTol(
                            sim.toDoubleArray(i, 0), ind.toDoubleArray(0), 1e-6, 1e-9));
                    c.match("SIMD asset " + (i + 1) + " aroon_down equals individual", Demo.sameTol(
        ...
        try (SimdResult sim = Aroon.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Aroon.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
                    c.match("SIMD option set " + (i + 1) + " aroon_up equals individual", Demo.sameTol(
                            sim.toDoubleArray(i, 0), ind.toDoubleArray(0), 1e-6, 1e-9));
                    c.match("SIMD option set " + (i + 1) + " aroon_down equals individual", Demo.sameTol(
                            sim.toDoubleArray(i, 1), ind.toDoubleArray(1), 1e-6, 1e-9));
                }
            }
        }
