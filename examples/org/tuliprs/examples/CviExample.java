...
public final class CviExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Cvi.minData(options) + 100;
        ...
        Outcome oc = Cvi.indicator(series, options, null);
        double[] fullCvi;
        try (Result res = oc.result()) {
            fullCvi = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Cvi.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continued = br.toDoubleArray(0);
                double[] tail = Arrays.copyOfRange(fullCvi, fullCvi.length - continued.length,
                        fullCvi.length);
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Cvi.ID);
                State rs = Cvi.deserializeState(blob);
        ...
        try (SimdResult sim = Cvi.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Cvi.indicator(assets[i], options, null);
        ...
        try (SimdResult sim = Cvi.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Cvi.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
