...
public final class MinExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Min.minData(options) + 100;
        ...
        // ---- full compute --------------------------------------------------
        System.out.println("\n=== full calculation ===");
        Outcome oc = Min.indicator(series, options, null);
        double[] fullMin;
        try (Result res = oc.result()) {
            fullMin = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Min.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continuedMin = br.toDoubleArray(0);
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Min.ID);
                State rs = Min.deserializeState(blob);
                ...
                try (rs; cl) {
                    ...
                    try (b1; b2; b3) {
                        c.match("deserialized state continues identically min",
        ...
        try (SimdResult sim = Min.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Min.indicator(assets[i], options, null);
        ...
        try (SimdResult sim = Min.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Min.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
