 ...
 */
public final class TrviExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Trvi.minData(options) + 100;
        ...
        Outcome oc = Trvi.indicator(series, options, new boolean[] {true, true});
        double[] fullTrvi;
        try (Result res = oc.result()) {
            fullTrvi = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Trvi.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continued = br.toDoubleArray(0);
                double[] tail = Arrays.copyOfRange(fullTrvi, fullTrvi.length - continued.length,
                        fullTrvi.length);
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Trvi.ID);
                State rs = Trvi.deserializeState(blob);
        ...
        try (SimdResult sim = Trvi.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Trvi.indicator(assets[i], options, null);
        ...
        try (SimdResult sim = Trvi.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Trvi.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
