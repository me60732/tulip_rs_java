...
public final class HmaExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Hma.minData(options) + 100;
        ...
        Outcome oc = Hma.indicator(series, options, null);
        double[] fullHma;
        try (Result res = oc.result()) {
            fullHma = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Hma.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continuedHma = br.toDoubleArray(0);
                double[] tailHma = Arrays.copyOfRange(fullHma, fullHma.length - continuedHma.length,
                        fullHma.length);
                c.match("partial+continued hma equals full recompute", Demo.same(tailHma, continuedHma));
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Hma.ID);
                State rs = Hma.deserializeState(blob);
                ...
                try (rs; cl) {
                    ...
                    try (b1; b2; b3) {
                        c.match("deserialized state continues identically hma",
                                Demo.same(b1.toDoubleArray(0), b2.toDoubleArray(0)));
                        c.match("cloned state continues identically hma",
        ...
        try (SimdResult sim = Hma.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Hma.indicator(assets[i], options, null);
                try (Result ind = r.result(); State st2 = r.state()) {
                    c.match("SIMD asset " + (i + 1) + " hma equals individual", Demo.sameTol(
        ...
        // ---- SIMD by options -------------------------------------------------
        System.out.println("\n=== SIMD by options (N=4) ===");
        double[][] optSets = {{8}, {10}, {12}, {15}};
        try (SimdResult sim = Hma.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Hma.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
                    c.match("SIMD option set " + (i + 1) + " hma equals individual", Demo.sameTol(
                            sim.toDoubleArray(i, 0), ind.toDoubleArray(0), 1e-6, 1e-9));
                }
            }
        }
