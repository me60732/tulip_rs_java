...
public final class EfExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Ef.minData(options) + 100;
        ...
        Outcome oc = Ef.indicator(series, options, null);
        double[] fullEf;
        try (Result res = oc.result()) {
            fullEf = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Ef.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continued = br.toDoubleArray(0);
                double[] tail = Arrays.copyOfRange(fullEf, fullEf.length - continued.length,
                        fullEf.length);
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Ef.ID);
                State rs = Ef.deserializeState(blob);
        ...
        try (SimdResult sim = Ef.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Ef.indicator(assets[i], options, null);
        ...
        try (SimdResult sim = Ef.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Ef.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
