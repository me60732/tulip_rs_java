 ...
 */
public final class CybercycleExample {

    public static void main(String[] args) {
        Demo.Check c = new Demo.Check();
        double[] options = {0.5}; // alpha

        Info info = Cybercycle.info();
        ...
        int n = 2 * (int) Cybercycle.minData(options) + 100;
        ...
        // ---- full compute --------------------------------------------------
        System.out.println("\n=== full calculation ===");
        Outcome oc = Cybercycle.indicator(series, options, null);
        double[] fullCybercycle;
        try (Result res = oc.result()) {
            fullCybercycle = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Cybercycle.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continuedCybercycle = br.toDoubleArray(0);
                double[] tailCybercycle = Arrays.copyOfRange(fullCybercycle, fullCybercycle.length - continuedCybercycle.length,
                        fullCybercycle.length);
                c.match("partial+continued cybercycle equals full recompute", Demo.same(tailCybercycle, continuedCybercycle));
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Cybercycle.ID);
                State rs = Cybercycle.deserializeState(blob);
                ...
                try (rs; cl) {
                    ...
                    try (b1; b2; b3) {
                        c.match("deserialized state continues identically cybercycle",
                                Demo.same(b1.toDoubleArray(0), b2.toDoubleArray(0)));
                        c.match("cloned state continues identically cybercycle",
        ...
        try (SimdResult sim = Cybercycle.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Cybercycle.indicator(assets[i], options, null);
                try (Result ind = r.result(); State st2 = r.state()) {
                    c.match("SIMD asset " + (i + 1) + " cybercycle equals individual", Demo.sameTol(
        ...
        // ---- SIMD by options -------------------------------------------------
        System.out.println("\n=== SIMD by options (N=4) ===");
        double[][] optSets = {{0.3}, {0.5}, {0.7}, {0.9}};
        try (SimdResult sim = Cybercycle.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Cybercycle.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
                    c.match("SIMD option set " + (i + 1) + " cybercycle equals individual", Demo.sameTol(
                            sim.toDoubleArray(i, 0), ind.toDoubleArray(0), 1e-6, 1e-9));
                }
            }
        }
