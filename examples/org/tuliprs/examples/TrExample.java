...
public final class TrExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Tr.minData(options) + 100;
        ...
        // ---- full compute --------------------------------------------------
        System.out.println("\n=== full calculation ===");
        Outcome oc = Tr.indicator(series);
        double[] fullTr;
        try (Result res = oc.result()) {
            fullTr = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Tr.indicator(slices(series, 0, partial));
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continuedTr = br.toDoubleArray(0);
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Tr.ID);
                State rs = Tr.deserializeState(blob);
        ...
        try (SimdResult sim = Tr.simdByAssets(assets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Tr.indicator(assets[i], null);
