...
public final class SmaExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Sma.minData(options) + 100;
        ...
        Outcome oc = Sma.indicator(series, options, null);
        double[] fullSma;
        try (Result res = oc.result()) {
            fullSma = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Sma.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continued = br.toDoubleArray(0);
                double[] tail = Arrays.copyOfRange(fullSma, fullSma.length - continued.length,
                        fullSma.length);
        ...
        try (SimdResult sim = Sma.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Sma.indicator(assets[i], options, null);
        ...
        try (SimdResult sim = Sma.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Sma.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
