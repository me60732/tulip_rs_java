 ...
 */
public final class TrixExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Trix.minData(options) + 100;
        ...
        Outcome oc = Trix.indicator(series, options, new boolean[] {true, true, true});
        double[] fullTrix;
        try (Result res = oc.result()) {
            fullTrix = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Trix.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continued = br.toDoubleArray(0);
                double[] tail = Arrays.copyOfRange(fullTrix, fullTrix.length - continued.length,
                        fullTrix.length);
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Trix.ID);
                State rs = Trix.deserializeState(blob);
        ...
        try (SimdResult sim = Trix.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Trix.indicator(assets[i], options, null);
        ...
        try (SimdResult sim = Trix.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Trix.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
