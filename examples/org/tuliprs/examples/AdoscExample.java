 ...
 */
public final class AdoscExample {

    public static void main(String[] args) {
        Demo.Check c = new Demo.Check();
        double[] options = {2.0, 10.0}; // short_period, long_period

        Info info = Adosc.info();
        ...
        int n = 2 * (int) Adosc.minData(options) + 100;
        ...
        Outcome oc = Adosc.indicator(series, options, null);
        double[] fullAdosc;
        try (Result res = oc.result()) {
            fullAdosc = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Adosc.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continued = br.toDoubleArray(0);
                double[] tail = Arrays.copyOfRange(fullAdosc, fullAdosc.length - continued.length,
                        fullAdosc.length);
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Adosc.ID);
                State rs = Adosc.deserializeState(blob);
        ...
        try (SimdResult sim = Adosc.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Adosc.indicator(assets[i], options, null);
        ...
        // ---- SIMD by options -------------------------------------------------
        System.out.println("\n=== SIMD by options (N=4) ===");
        double[][] optSets = {{2, 10}, {3, 11}, {5, 13}, {7, 15}};
        try (SimdResult sim = Adosc.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Adosc.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
