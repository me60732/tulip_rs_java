 ...
 */
public final class WadExample {

    public static void main(String[] args) {
        Demo.Check c = new Demo.Check();
        double[] options = new double[0]; // no options
        ...
        int n = 2 * (int) Wad.minData(options) + 100;
        ...
        Outcome oc = Wad.indicator(series, options);
        double[] fullWad;
        try (Result res = oc.result()) {
            fullWad = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Wad.indicator(slices(series, 0, partial), options);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continuedWad = br.toDoubleArray(0);
                double[] tailWad = Arrays.copyOfRange(fullWad, fullWad.length - continuedWad.length,
                        fullWad.length);
                c.match("partial+continued wad equals full recompute", Demo.same(tailWad, continuedWad));
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Wad.ID);
                State rs = Wad.deserializeState(blob);
                ...
                try (rs; cl) {
                    ...
                    try (b1; b2; b3) {
                        c.match("deserialized state continues identically wad",
                                Demo.same(b1.toDoubleArray(0), b2.toDoubleArray(0)));
                        c.match("cloned state continues identically wad",
        ...
        try (SimdResult sim = Wad.simdByAssets(assets, options)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Wad.indicator(assets[i], options);
                try (Result ind = r.result(); State st2 = r.state()) {
                    c.match("SIMD asset " + (i + 1) + " wad equals individual", Demo.sameTol(
