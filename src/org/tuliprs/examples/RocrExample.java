 ...
 */
public final class RocrExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Rocr.minData(options) + 100;
        ...
        Outcome oc = Rocr.indicator(series, options, null);
        double[] fullRocr;
        try (Result res = oc.result()) {
            fullRocr = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Rocr.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continuedRocr = br.toDoubleArray(0);
                double[] tailRocr = Arrays.copyOfRange(fullRocr, fullRocr.length - continuedRocr.length,
                        fullRocr.length);
                c.match("partial+continued equals full recompute", Demo.same(tailRocr, continuedRocr));
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Rocr.ID);
                State rs = Rocr.deserializeState(blob);
        ...
        try (SimdResult sim = Rocr.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Rocr.indicator(assets[i], options, null);
        ...
        try (SimdResult sim = Rocr.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Rocr.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
