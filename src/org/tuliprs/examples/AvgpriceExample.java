 ...
 */
public final class AvgpriceExample {

    public static void main(String[] args) {
        Demo.Check c = new Demo.Check();
        double[] options = new double[0]; // none

        Info info = Avgprice.info();
        ...
        int n = 2 * (int) Avgprice.minData(options) + 100;
        ...
        Outcome oc = Avgprice.indicator(series, options, null);
        double[] fullAvgprice;
        try (Result res = oc.result()) {
            fullAvgprice = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Avgprice.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continued = br.toDoubleArray(0);
                double[] tail = Arrays.copyOfRange(fullAvgprice, fullAvgprice.length - continued.length,
                        fullAvgprice.length);
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Avgprice.ID);
                State rs = Avgprice.deserializeState(blob);
        ...
        double[][] scaled = slices(new double[][] {
                Demo.scale(open, 1.2), Demo.scale(high, 1.2),
                Demo.scale(low, 1.2), Demo.scale(close, 1.2)}, 0, n);
        double[][][] assets = {series, scaled};
        try (SimdResult sim = Avgprice.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Avgprice.indicator(assets[i], options, null);
        ...
        // ---- SIMD by options omitted: no options available in the FFI
