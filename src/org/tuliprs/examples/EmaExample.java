...
public final class EmaExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Ema.minData(options) + 100;
        ...
        // ---- full compute --------------------------------------------------
        System.out.println("\n=== full calculation ===");
        Outcome oc = Ema.indicator(series, options, null);
        double[] fullEma;
        try (Result res = oc.result()) {
            fullEma = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Ema.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continuedEma = br.toDoubleArray(0);
                double[] tail = Arrays.copyOfRange(fullEma, fullEma.length - continuedEma.length,
                        fullEma.length);
                c.match("partial+continued equals full recompute", Demo.same(tail, continuedEma));
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Ema.ID);
                State rs = Ema.deserializeState(blob);
        ...
        try (SimdResult sim = Ema.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Ema.indicator(assets[i], options, null);
        ...
        try (SimdResult sim = Ema.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Ema.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
