 ...
 */
public final class AdxrExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Adxr.minData(options) + 100;
        ...
        Outcome oc = Adxr.indicator(series, options, null);
        double[] fullAdxr;
        try (Result res = oc.result()) {
            fullAdxr = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Adxr.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continued = br.toDoubleArray(0);
                double[] tail = Arrays.copyOfRange(fullAdxr, fullAdxr.length - continued.length,
                        fullAdxr.length);
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Adxr.ID);
                State rs = Adxr.deserializeState(blob);
        ...
        try (SimdResult sim = Adxr.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Adxr.indicator(assets[i], options, null);
        ...
        try (SimdResult sim = Adxr.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Adxr.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
