...
public final class CmoExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Cmo.minData(options) + 100;
        ...
        Outcome oc = Cmo.indicator(series, options, null);
        double[] fullCmo;
        try (Result res = oc.result()) {
            fullCmo = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Cmo.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continued = br.toDoubleArray(0);
                double[] tail = Arrays.copyOfRange(fullCmo, fullCmo.length - continued.length,
                        fullCmo.length);
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Cmo.ID);
                State rs = Cmo.deserializeState(blob);
        ...
        try (SimdResult sim = Cmo.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Cmo.indicator(assets[i], options, null);
        ...
        try (SimdResult sim = Cmo.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Cmo.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
