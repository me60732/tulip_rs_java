...
public final class DpoExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Dpo.minData(options) + 100;
        ...
        Outcome oc = Dpo.indicator(series, options, null);
        double[] fullDpo;
        try (Result res = oc.result()) {
            fullDpo = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Dpo.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continued = br.toDoubleArray(0);
                double[] tail = Arrays.copyOfRange(fullDpo, fullDpo.length - continued.length,
                        fullDpo.length);
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Dpo.ID);
                State rs = Dpo.deserializeState(blob);
        ...
        try (SimdResult sim = Dpo.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Dpo.indicator(assets[i], options, null);
        ...
        try (SimdResult sim = Dpo.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Dpo.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
