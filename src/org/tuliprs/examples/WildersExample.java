...
public final class WildersExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Wilders.minData(options) + 100;
        ...
        Outcome oc = Wilders.indicator(series, options);
        double[] fullWilders;
        try (Result res = oc.result()) {
            fullWilders = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Wilders.indicator(slices(series, 0, partial), options);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continued = br.toDoubleArray(0);
                double[] tail = Arrays.copyOfRange(fullWilders, fullWilders.length - continued.length,
                        fullWilders.length);
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Wilders.ID);
                State rs = Wilders.deserializeState(blob);
        ...
        try (SimdResult sim = Wilders.simdByAssets(assets, options)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Wilders.indicator(assets[i], options);
        ...
        try (SimdResult sim = Wilders.simdByOptions(series, optSets)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Wilders.indicator(series, optSets[i]);
                try (Result ind = r.result(); State st2 = r.state()) {
