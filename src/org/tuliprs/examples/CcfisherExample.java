 ...
 */
public final class CcfisherExample {

    public static void main(String[] args) {
        Demo.Check c = new Demo.Check();
        double[] options = {0.1}; // alpha

        Info info = Ccfisher.info();
        ...
        int n = 2 * (int) Ccfisher.minData(options) + 100;
        ...
        Outcome oc = Ccfisher.indicator(series, options, new boolean[] {true, true, true});
        ...
        int partial = n - 50;
        Outcome p = Ccfisher.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            ...
            System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                    blob.length, Ccfisher.ID);
            State rs = Ccfisher.deserializeState(blob);
        ...
        try (SimdResult sim = Ccfisher.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Ccfisher.indicator(assets[i], options, null);
        ...
        // ---- SIMD by options -------------------------------------------------
        System.out.println("\n=== SIMD by options (N=4) ===");
        double[][] optSets = {{0.05}, {0.1}, {0.2}, {0.3}};
        try (SimdResult sim = Ccfisher.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Ccfisher.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
                    for (int o = 0; o < ind.numOutputs(); o++) {
