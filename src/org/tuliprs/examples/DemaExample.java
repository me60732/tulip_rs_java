 ...
 */
public final class DemaExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Dema.minData(options) + 100;
        ...
        // ---- full compute --------------------------------------------------
        System.out.println("\n=== full calculation ===");
        Outcome oc = Dema.indicator(series, options, null);
        double[] fullDema;
        try (Result res = oc.result()) {
            fullDema = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Dema.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continuedDema = br.toDoubleArray(0);
                double[] tailDema = Arrays.copyOfRange(fullDema, fullDema.length - continuedDema.length,
                        fullDema.length);
                c.match("partial+continued dema equals full recompute", Demo.same(tailDema, continuedDema));
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Dema.ID);
                State rs = Dema.deserializeState(blob);
                ...
                try (rs; cl) {
                    ...
                    try (b1; b2; b3) {
                        c.match("deserialized state continues identically dema",
                                Demo.same(b1.toDoubleArray(0), b2.toDoubleArray(0)));
                        c.match("cloned state continues identically dema",
        ...
        try (SimdResult sim = Dema.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Dema.indicator(assets[i], options, null);
                try (Result ind = r.result(); State st2 = r.state()) {
                    c.match("SIMD asset " + (i + 1) + " dema equals individual", Demo.sameTol(
        ...
        try (SimdResult sim = Dema.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Dema.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
                    c.match("SIMD option set " + (i + 1) + " dema equals individual", Demo.sameTol(
                            sim.toDoubleArray(i, 0), ind.toDoubleArray(0), 1e-6, 1e-9));
                }
            }
        }
