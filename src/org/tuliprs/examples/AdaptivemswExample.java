 ...
 */
public final class AdaptivemswExample {

    public static void main(String[] args) {
        Demo.Check c = new Demo.Check();
        double[] options = new double[0];

        Info info = Adaptivemsw.info();
        ...
        int n = 2 * (int) Adaptivemsw.minData(options) + 100;
        ...
        Outcome oc = Adaptivemsw.indicator(series, options, null);
        double[] fullAdaptivemsw;
        try (Result res = oc.result()) {
            fullAdaptivemsw = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Adaptivemsw.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continued = br.toDoubleArray(0);
                double[] tail = Arrays.copyOfRange(fullAdaptivemsw, fullAdaptivemsw.length - continued.length,
                        fullAdaptivemsw.length);
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Adaptivemsw.ID);
                State rs = Adaptivemsw.deserializeState(blob);
        ...
        try (SimdResult sim = Adaptivemsw.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Adaptivemsw.indicator(assets[i], options, null);
