...
public final class AdExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Ad.minData(options) + 100;
        ...
        Outcome oc = Ad.indicator(series, options, null);
        double[] fullAd;
        try (Result res = oc.result()) {
            fullAd = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Ad.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continued = br.toDoubleArray(0);
                double[] tail = Arrays.copyOfRange(fullAd, fullAd.length - continued.length,
                        fullAd.length);
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Ad.ID);
                State rs = Ad.deserializeState(blob);
        ...
        try (SimdResult sim = Ad.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Ad.indicator(assets[i], options, null);
