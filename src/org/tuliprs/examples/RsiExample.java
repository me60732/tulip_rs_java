...
public final class RsiExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Rsi.minData(options) + 100;
        ...
        Outcome oc = Rsi.indicator(series, options, null);
        double[] fullRsi;
        try (Result res = oc.result()) {
            fullRsi = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Rsi.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continued = br.toDoubleArray(0);
                double[] tail = Arrays.copyOfRange(fullRsi, fullRsi.length - continued.length,
                        fullRsi.length);
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Rsi.ID);
                State rs = Rsi.deserializeState(blob);
        ...
        try (SimdResult sim = Rsi.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Rsi.indicator(assets[i], options, null);
        ...
        try (SimdResult sim = Rsi.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Rsi.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
