...
/**
 * Stoch example: full compute, streaming continuation, state persistence
 * (serialize / deserialize / duplicate), and both SIMD modes — the Java
 * mirror of the Go/C examples, with every step verified.
 */
public final class StochExample {

    public static void main(String[] args) {
        Demo.Check c = new Demo.Check();
        double[] options = {14.0, 3.0, 3.0}; // k_period, k_slow_period, d_period

        Info info = Stoch.info();
        ...
        int n = 2 * (int) Stoch.minData(options) + 100;
        ...
        Outcome oc = Stoch.indicator(series, options, null);
        double[] fullK;
        double[] fullD;
        try (Result res = oc.result()) {
            fullK = res.toDoubleArray(0);
            fullD = res.toDoubleArray(1);
        ...
        int partial = n - 50;
        Outcome p = Stoch.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continuedK = br.toDoubleArray(0);
                double[] continuedD = br.toDoubleArray(1);
                double[] tailK = Arrays.copyOfRange(fullK, fullK.length - continuedK.length,
                        fullK.length);
                double[] tailD = Arrays.copyOfRange(fullD, fullD.length - continuedD.length,
                        fullD.length);
                c.match("partial+continued k equals full recompute", Demo.same(tailK, continuedK));
                c.match("partial+continued d equals full recompute", Demo.same(tailD, continuedD));
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Stoch.ID);
                State rs = Stoch.deserializeState(blob);
                ...
                try (rs; cl) {
                    ...
                    try (b1; b2; b3) {
                        c.match("deserialized state continues identically k",
                                Demo.same(b1.toDoubleArray(0), b2.toDoubleArray(0)));
                        c.match("cloned state continues identically k",
                                Demo.same(b1.toDoubleArray(0), b3.toDoubleArray(0)));
                        c.match("cloned state continues identically d",
        ...
        try (SimdResult sim = Stoch.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Stoch.indicator(assets[i], options, null);
                try (Result ind = r.result(); State st2 = r.state()) {
                    c.match("SIMD asset " + (i + 1) + " k equals individual", Demo.sameTol(
                            sim.toDoubleArray(i, 0), ind.toDoubleArray(0), 1e-6, 1e-9));
                    c.match("SIMD asset " + (i + 1) + " d equals individual", Demo.sameTol(
        ...
        // ---- SIMD by options -------------------------------------------------
        System.out.println("\n=== SIMD by options (N=4) ===");
        double[][] optSets = {{10, 3, 3}, {14, 3, 3}, {20, 3, 3}, {25, 3, 3}};
        try (SimdResult sim = Stoch.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Stoch.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
                    c.match("SIMD option set " + (i + 1) + " k equals individual", Demo.sameTol(
                            sim.toDoubleArray(i, 0), ind.toDoubleArray(0), 1e-6, 1e-9));
                    c.match("SIMD option set " + (i + 1) + " d equals individual", Demo.sameTol(
                            sim.toDoubleArray(i, 1), ind.toDoubleArray(1), 1e-6, 1e-9));
                }
            }
        }
