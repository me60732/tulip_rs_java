 ...
 */
public final class KamaExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Kama.minData(options) + 100;
        ...
        Outcome oc = Kama.indicator(series, options, new boolean[] {true});
        double[] fullKama;
        try (Result res = oc.result()) {
            fullKama = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Kama.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continued = br.toDoubleArray(0);
                double[] tail = Arrays.copyOfRange(fullKama, fullKama.length - continued.length,
                        fullKama.length);
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Kama.ID);
                State rs = Kama.deserializeState(blob);
        ...
        try (SimdResult sim = Kama.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Kama.indicator(assets[i], options, null);
        ...
        try (SimdResult sim = Kama.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Kama.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
