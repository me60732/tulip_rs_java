...
public final class ApoExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Apo.minData(options) + 100;
        ...
        Outcome oc = Apo.indicator(series, options, null);
        double[] fullApo;
        try (Result res = oc.result()) {
            fullApo = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Apo.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continued = br.toDoubleArray(0);
                double[] tail = Arrays.copyOfRange(fullApo, fullApo.length - continued.length,
                        fullApo.length);
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Apo.ID);
                State rs = Apo.deserializeState(blob);
        ...
        try (SimdResult sim = Apo.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Apo.indicator(assets[i], options, null);
        ...
        // ---- SIMD by options -------------------------------------------------
        System.out.println("\n=== SIMD by options (N=4) ===");
        double[][] optSets = {{3, 10}, {5, 10}, {7, 10}, {10, 20}};
        try (SimdResult sim = Apo.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Apo.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
