 ...
 */
public final class VoscExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Vosc.minData(options) + 100;
        String[] names = info.inputs().toArray(new String[0]);
        double[][] series = Demo.seriesFor(names, n);
        double[] volume = series[0];

        // ---- full compute --------------------------------------------------
        System.out.println("\n=== full calculation (all optional outputs) ===");
        Outcome oc = Vosc.indicator(series, options, new boolean[] {true, true});
        double[] fullVosc;
        try (Result res = oc.result()) {
            fullVosc = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Vosc.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continuedVosc = br.toDoubleArray(0);
                double[] tailVosc = Arrays.copyOfRange(fullVosc, fullVosc.length - continuedVosc.length,
                        fullVosc.length);
                c.match("partial+continued vosc equals full recompute", Demo.same(tailVosc, continuedVosc));
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Vosc.ID);
                State rs = Vosc.deserializeState(blob);
                ...
                try (rs; cl) {
                    ...
                    try (b1; b2; b3) {
                        c.match("deserialized state continues identically vosc",
                                Demo.same(b1.toDoubleArray(0), b2.toDoubleArray(0)));
                        c.match("cloned state continues identically vosc",
        ...
        // ---- SIMD by assets --------------------------------------------------
        System.out.println("\n=== SIMD by assets (N=2) ===");
        double[][] scaled = slices(new double[][] {Demo.scale(volume, 1.2)}, 0, n);
        double[][][] assets = {series, scaled};
        try (SimdResult sim = Vosc.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Vosc.indicator(assets[i], options, null);
                try (Result ind = r.result(); State st2 = r.state()) {
                    c.match("SIMD asset " + (i + 1) + " vosc equals individual", Demo.sameTol(
        ...
        // ---- SIMD by options -------------------------------------------------
        System.out.println("\n=== SIMD by options (N=4) ===");
        double[][] optSets = {{5, 20}, {10, 30}, {15, 40}, {20, 50}};
        try (SimdResult sim = Vosc.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Vosc.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
                    c.match("SIMD option set " + (i + 1) + " vosc equals individual", Demo.sameTol(
                            sim.toDoubleArray(i, 0), ind.toDoubleArray(0), 1e-6, 1e-9));
                }
            }
        }
