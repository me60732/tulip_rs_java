 ...
 * (serialize / deserialize / duplicate), and both SIMD modes — every step
 * verified. (The live FFI exposes psar with zero optional outputs; the
 * single-row surface here matches {@code Psar.info().optionalOutputs()}.)
 */
public final class PsarExample {

    public static void main(String[] args) {
        Demo.Check c = new Demo.Check();
        double[] options = {0.02, 0.2}; // acceleration_factor, maximum
        ...
        int n = 2 * (int) Psar.minData(options) + 100;
        ...
        // ---- full compute --------------------------------------------------
        System.out.println("\n=== full calculation ===");
        Outcome oc = Psar.indicator(series, options);
        double[] fullPsar;
        try (Result res = oc.result()) {
            fullPsar = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Psar.indicator(slices(series, 0, partial), options);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continued = br.toDoubleArray(0);
                double[] tail = Arrays.copyOfRange(fullPsar, fullPsar.length - continued.length,
                        fullPsar.length);
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Psar.ID);
                State rs = Psar.deserializeState(blob);
        ...
        try (SimdResult sim = Psar.simdByAssets(assets, options)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Psar.indicator(assets[i], options);
        ...
        // ---- SIMD by options -------------------------------------------------
        System.out.println("\n=== SIMD by options (N=4) ===");
        double[][] optSets = {{0.01, 0.2}, {0.02, 0.2}, {0.03, 0.2}, {0.05, 0.2}};
        try (SimdResult sim = Psar.simdByOptions(series, optSets)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Psar.indicator(series, optSets[i]);
                try (Result ind = r.result(); State st2 = r.state()) {
