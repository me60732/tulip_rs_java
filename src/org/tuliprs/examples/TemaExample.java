 ...
 */
public final class TemaExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Tema.minData(options) + 100;
        ...
        // ---- full compute --------------------------------------------------
        System.out.println("\n=== full calculation ===");
        Outcome oc = Tema.indicator(series, options);
        double[] fullTema;
        try (Result res = oc.result()) {
            fullTema = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Tema.indicator(slices(series, 0, partial), options);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continuedTema = br.toDoubleArray(0);
                double[] tail = Arrays.copyOfRange(fullTema, fullTema.length - continuedTema.length,
                        fullTema.length);
                c.match("partial+continued equals full recompute", Demo.same(tail, continuedTema));
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Tema.ID);
                State rs = Tema.deserializeState(blob);
        ...
        try (SimdResult sim = Tema.simdByAssets(assets, options)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Tema.indicator(assets[i], options);
        ...
        try (SimdResult sim = Tema.simdByOptions(series, optSets)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Tema.indicator(series, optSets[i]);
                try (Result ind = r.result(); State st2 = r.state()) {
