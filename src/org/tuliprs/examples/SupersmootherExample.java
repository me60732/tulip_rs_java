...
public final class SupersmootherExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Supersmoother.minData(options) + 100;
        ...
        // ---- full compute --------------------------------------------------
        System.out.println("\n=== full calculation ===");
        Outcome oc = Supersmoother.indicator(series, options);
        double[] fullSupersmoother;
        try (Result res = oc.result()) {
            fullSupersmoother = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Supersmoother.indicator(slices(series, 0, partial), options);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continuedSupersmoother = br.toDoubleArray(0);
                double[] tail = Arrays.copyOfRange(fullSupersmoother, fullSupersmoother.length - continuedSupersmoother.length,
                        fullSupersmoother.length);
                c.match("partial+continued equals full recompute", Demo.same(tail, continuedSupersmoother));
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Supersmoother.ID);
                State rs = Supersmoother.deserializeState(blob);
        ...
        try (SimdResult sim = Supersmoother.simdByAssets(assets, options)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Supersmoother.indicator(assets[i], options);
        ...
        // ---- SIMD by options -------------------------------------------------
        System.out.println("\n=== SIMD by options (N=4) ===");
        double[][] optSets = {{15}, {20}, {25}, {30}};
        try (SimdResult sim = Supersmoother.simdByOptions(series, optSets)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Supersmoother.indicator(series, optSets[i]);
                try (Result ind = r.result(); State st2 = r.state()) {
