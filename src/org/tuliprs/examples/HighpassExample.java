...
public final class HighpassExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Highpass.minData(options) + 100;
        ...
        Outcome oc = Highpass.indicator(series, options, null);
        double[] fullHighpass;
        try (Result res = oc.result()) {
            fullHighpass = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Highpass.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continuedHighpass = br.toDoubleArray(0);
                double[] tailHighpass = Arrays.copyOfRange(fullHighpass, fullHighpass.length - continuedHighpass.length,
                        fullHighpass.length);
                c.match("partial+continued equals full recompute", Demo.same(tailHighpass, continuedHighpass));
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Highpass.ID);
                State rs = Highpass.deserializeState(blob);
        ...
        try (SimdResult sim = Highpass.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Highpass.indicator(assets[i], options, null);
        ...
        try (SimdResult sim = Highpass.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Highpass.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
    ...
    /** Mandatory output name first (no optional outputs for highpass). */
