...
public final class ZlemaExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Zlema.minData(options) + 100;
        ...
        Outcome oc = Zlema.indicator(series, options);
        double[] fullZlema;
        try (Result res = oc.result()) {
            fullZlema = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Zlema.indicator(slices(series, 0, partial), options);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continued = br.toDoubleArray(0);
                double[] tail = Arrays.copyOfRange(fullZlema, fullZlema.length - continued.length,
                        fullZlema.length);
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Zlema.ID);
                State rs = Zlema.deserializeState(blob);
        ...
        try (SimdResult sim = Zlema.simdByAssets(assets, options)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Zlema.indicator(assets[i], options);
        ...
        try (SimdResult sim = Zlema.simdByOptions(series, optSets)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Zlema.indicator(series, optSets[i]);
                try (Result ind = r.result(); State st2 = r.state()) {
