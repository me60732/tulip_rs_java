...
public final class DxExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Dx.minData(options) + 100;
        ...
        Outcome oc = Dx.indicator(series, options, null);
        double[] fullDx;
        try (Result res = oc.result()) {
            fullDx = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Dx.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continued = br.toDoubleArray(0);
                double[] tail = Arrays.copyOfRange(fullDx, fullDx.length - continued.length,
                        fullDx.length);
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Dx.ID);
                State rs = Dx.deserializeState(blob);
        ...
        try (SimdResult sim = Dx.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Dx.indicator(assets[i], options, null);
        ...
        try (SimdResult sim = Dx.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Dx.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
