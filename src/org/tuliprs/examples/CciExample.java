...
public final class CciExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Cci.minData(options) + 100;
        ...
        Outcome oc = Cci.indicator(series, options, new boolean[] {true, true, true});
        ...
        int partial = n - 50;
        Outcome p = Cci.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            ...
            System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                    blob.length, Cci.ID);
            State rs = Cci.deserializeState(blob);
        ...
        try (SimdResult sim = Cci.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Cci.indicator(assets[i], options, null);
        ...
        try (SimdResult sim = Cci.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Cci.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
                    for (int o = 0; o < ind.numOutputs(); o++) {
