...
public final class DmExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Dm.minData(options) + 100;
        ...
        // ---- full compute --------------------------------------------------
        System.out.println("\n=== full calculation ===");
        Outcome oc = Dm.indicator(series, options, null);
        try (Result res = oc.result()) {
            int nout = res.numOutputs();
            System.out.println("DEBUG numOutputs: " + nout);
            String[] rowNames = rowNames(info);
            for (int i = 0; i < nout; i++) {
                long len = res.rowLength(i);
                double[] arr = res.toDoubleArray(i);
                System.out.printf("  %-6s %d values%n", rowNames[i], len);
            }
            DmExample.fullReference = res.toArrays();
        }

        // ---- partial + batch continuation ----------------------------------
        System.out.println("\n=== partial calculation + batch continuation ===");
        int partial = n - 50;
        Outcome p = Dm.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continuedPlusDm = br.toDoubleArray(0);
                double[] continuedMinusDm = br.toDoubleArray(1);
                String[] rowNames = rowNames(info);
                double[] tailPlusDm = Arrays.copyOfRange(
                        DmExample.fullReference[0],
                        DmExample.fullReference[0].length - continuedPlusDm.length,
                        DmExample.fullReference[0].length);
                double[] tailMinusDm = Arrays.copyOfRange(
                        DmExample.fullReference[1],
                        DmExample.fullReference[1].length - continuedMinusDm.length,
                        DmExample.fullReference[1].length);
                c.match("partial+continued plus_dm equals full recompute", Demo.same(tailPlusDm, continuedPlusDm));
                c.match("partial+continued minus_dm equals full recompute", Demo.same(tailMinusDm, continuedMinusDm));
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Dm.ID);
                State rs = Dm.deserializeState(blob);
                ...
                try (rs; cl) {
                    ...
                    try (b1; b2; b3) {
                        c.match("deserialized state continues identically plus_dm",
                                Demo.same(b1.toDoubleArray(0), b2.toDoubleArray(0)));
                        c.match("cloned state continues identically plus_dm",
                                Demo.same(b1.toDoubleArray(0), b3.toDoubleArray(0)));
                        c.match("deserialized state continues identically minus_dm",
                                Demo.same(b1.toDoubleArray(1), b2.toDoubleArray(1)));
                        c.match("cloned state continues identically minus_dm",
        ...
        try (SimdResult sim = Dm.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Dm.indicator(assets[i], options, null);
                try (Result ind = r.result(); State st2 = r.state()) {
                    c.match("SIMD asset " + (i + 1) + " plus_dm equals individual", Demo.sameTol(
                            sim.toDoubleArray(i, 0), ind.toDoubleArray(0), 1e-6, 1e-9));
                    c.match("SIMD asset " + (i + 1) + " minus_dm equals individual", Demo.sameTol(
        ...
        try (SimdResult sim = Dm.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Dm.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
                    c.match("SIMD option set " + (i + 1) + " plus_dm equals individual", Demo.sameTol(
                            sim.toDoubleArray(i, 0), ind.toDoubleArray(0), 1e-6, 1e-9));
                    c.match("SIMD option set " + (i + 1) + " minus_dm equals individual", Demo.sameTol(
                            sim.toDoubleArray(i, 1), ind.toDoubleArray(1), 1e-6, 1e-9));
                }
            }
        }
    ...
    /** Full reference outputs for continuation comparison. */
    private static double[][] fullReference;
