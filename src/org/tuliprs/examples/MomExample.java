...
public final class MomExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Mom.minData(options) + 100;
        ...
        Outcome oc = Mom.indicator(series, options, null);
        double[] fullMom;
        try (Result res = oc.result()) {
            fullMom = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Mom.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continued = br.toDoubleArray(0);
                double[] tail = Arrays.copyOfRange(fullMom, fullMom.length - continued.length,
                        fullMom.length);
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Mom.ID);
                State rs = Mom.deserializeState(blob);
        ...
        try (SimdResult sim = Mom.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Mom.indicator(assets[i], options, null);
        ...
        try (SimdResult sim = Mom.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Mom.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
