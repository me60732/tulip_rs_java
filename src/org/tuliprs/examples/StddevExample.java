...
public final class StddevExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Stddev.minData(options) + 100;
        ...
        // ---- full compute --------------------------------------------------
        System.out.println("\n=== full calculation ===");
        Outcome oc = Stddev.indicator(series, options, null);
        double[] fullStddev;
        try (Result res = oc.result()) {
            fullStddev = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Stddev.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continued = br.toDoubleArray(0);
                double[] tail = Arrays.copyOfRange(fullStddev, fullStddev.length - continued.length,
                        fullStddev.length);
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Stddev.ID);
                State rs = Stddev.deserializeState(blob);
        ...
        try (SimdResult sim = Stddev.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Stddev.indicator(assets[i], options, null);
        ...
        try (SimdResult sim = Stddev.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Stddev.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
