...
public final class RoofingfilterExample {
    ...
    public static void main(String[] args) {
        ...
        int n = 2 * (int) Roofingfilter.minData(options) + 100;
        ...
        Outcome oc = Roofingfilter.indicator(series, options, null);
        double[] fullRoofing;
        try (Result res = oc.result()) {
            fullRoofing = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Roofingfilter.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continued = br.toDoubleArray(0);
                double[] tail = Arrays.copyOfRange(fullRoofing, fullRoofing.length - continued.length,
                        fullRoofing.length);
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Roofingfilter.ID);
                State rs = Roofingfilter.deserializeState(blob);
        ...
        try (SimdResult sim = Roofingfilter.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Roofingfilter.indicator(assets[i], options, null);
        ...
        // ---- SIMD by options -------------------------------------------------
        System.out.println("\n=== SIMD by options (N=4) ===");
        double[][] optSets = {{5, 10}, {10, 20}, {15, 30}, {20, 40}};
        try (SimdResult sim = Roofingfilter.simdByOptions(series, optSets, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Roofingfilter.indicator(series, optSets[i], null);
                try (Result ind = r.result(); State st2 = r.state()) {
