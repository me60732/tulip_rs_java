 ...
 */
public final class HomodynediscriminatorExample {

    public static void main(String[] args) {
        Demo.Check c = new Demo.Check();
        double[] options = {}; // none

        Info info = Homodynediscriminator.info();
        ...
        int n = 2 * (int) Homodynediscriminator.minData(options) + 100;
        ...
        Outcome oc = Homodynediscriminator.indicator(series, options, null);
        double[] fullDcPeriod;
        try (Result res = oc.result()) {
            fullDcPeriod = res.toDoubleArray(0);
        ...
        int partial = n - 50;
        Outcome p = Homodynediscriminator.indicator(slices(series, 0, partial), options, null);
        try (Result pr = p.result(); State pst = p.state()) {
            Result br = pst.batch(slices(series, partial, n));
            try (br) {
                double[] continuedDcPeriod = br.toDoubleArray(0);
                double[] tailDcPeriod = Arrays.copyOfRange(fullDcPeriod, fullDcPeriod.length - continuedDcPeriod.length,
                        fullDcPeriod.length);
                c.match("partial+continued dc_period equals full recompute", Demo.same(tailDcPeriod, continuedDcPeriod));
                ...
                System.out.printf("  bincode blob: %d bytes (indicator id 0x%08x)%n",
                        blob.length, Homodynediscriminator.ID);
                State rs = Homodynediscriminator.deserializeState(blob);
                ...
                try (rs; cl) {
                    ...
                    try (b1; b2; b3) {
                        c.match("deserialized state continues identically dc_period",
                                Demo.same(b1.toDoubleArray(0), b2.toDoubleArray(0)));
                        c.match("cloned state continues identically dc_period",
        ...
        try (SimdResult sim = Homodynediscriminator.simdByAssets(assets, options, null)) {
            for (int i = 0; i < sim.numResults(); i++) {
                Outcome r = Homodynediscriminator.indicator(assets[i], options, null);
                try (Result ind = r.result(); State st2 = r.state()) {
                    c.match("SIMD asset " + (i + 1) + " dc_period equals individual", Demo.sameTol(
